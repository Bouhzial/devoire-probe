...
class SimpleForm extends Component {
  ...
  onSubmitForm() {
    ...
    if (this.state.order === true && this.state.remis === true && y > 0) {
      ...
      this.state.order === true &&
      this.state.remis === true &&
      y === 0
    ) {
      this.setState({ isK: !this.state.isK });
      if (this.state.k > this.state.n) {
        resultat = " k> n c'est imposible";
      } else {
        resultat = this.factorial(this.state.n) / this.factorial(this.state.k);
        typ = "Permutation";
        remi = "avec remis ";
      }
      this.setState({ k: "" });
      ...
      resultat =
        this.factorial(this.state.n) /
        (this.factorial(this.state.p) * this.factorial(y));
      typ = "Combinaison";
      remi = "sans remis ";
    } else if (
      this.state.order === false &&
      ...
      resultat =
        this.factorial(y1) /
        (this.factorial(this.state.p) * this.factorial(this.state.n - 1));
      typ = "Combinaison";
      remi = "avec remis ";
    } else {
      resultat = "imposible de calcule";
      typ = "error";
    }
    ...
    return (
      <div className="test">
        ...
        <div>
          {this.state.isK ? (
            <label>
              Dans le cas où il existerait plusieurs répétitions k d’un même
              objet parmi les n objets.
              <input
                className="cb"
                name="k"
        ...
        <div>
          <p>
            le type de calcule est : {this.state.type} {this.state.avec}{" "}
          </p>
        </div>
        <div>
